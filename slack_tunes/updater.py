...
def osascript(player, command):
    command = 'tell application "{0}" to {1} as string'.format(player, command)
    command = "osascript -e '{0}'".format(command)
    return (
        subprocess.check_output(command, shell=True).decode(sys.stdout.encoding).strip()
    )


def is_running(player):
    command = 'if application "{0}" is running then "running"'.format(player)
    command = "osascript -e '{0}'".format(command)
    try:
        return (
...
def spotify_song():
    ...
    try:
        song = osascript(
            "Spotify",
            'if player state is playing then artist of current track & " __SLACK_TUNES_DELIMITER__ " & name of current track',
        )  # pep8
    except subprocess.CalledProcessError:
        song = ""
...
def itunes_song(catalina=False):
    ...
    try:
        return osascript(
            app,
            'if player state is playing then artist of current track & " - " & name of current track',
        )  # pep8
    except subprocess.CalledProcessError:
        return None
